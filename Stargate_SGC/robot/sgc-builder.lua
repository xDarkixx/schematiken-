...
local function loadConfig(path)
  ...
  -- config.cfg returns globals in this project, so normalize them here.
  return {
    ...
    RESERVED_SLOTS = RESERVED_SLOTS or {1},
    TOOL_SLOTS = TOOL_SLOTS or {1,2,3,4,5,6,7,8,9},
    MOVE_RETRIES = MOVE_RETRIES or 3,
    PLACE_RETRIES = PLACE_RETRIES or 3,
    SAVE_EVERY = SAVE_EVERY or 10,
    UNLOAD_WHEN_FULL = UNLOAD_WHEN_FULL ~= false
  }
end
...
local function placeAt(b)
  ...
  if b.face ~= nil then nav:face(b.face) end
  robot.select(b.slot)
...
if not plan then
  say(err)
  say("Expected a validated .plan generated from the SGC schematic source.")
  return
end
...
if okTool then say("Tool: slot " .. toolSlot .. " (" .. toolName .. ")")
else say("Warning: no pickaxe/tool detected; continuing for placement-only plans") end

for i = state.index, #plan.blocks do
  local b = plan.blocks[i]
  if not b.protected then
    local ok, reason = nav:goTo(b.x, b.y, b.z)
    if not ok then
      save(i)
      say("STOP at block " .. i .. ": " .. tostring(reason))
      return
    end
    if not placeAt(b) then
      save(i)
      say("STOP: placement failed at block " .. i)
      return
    end
  end

  if cfg.UNLOAD_WHEN_FULL and not inv:hasSpace() then
    say("Inventory full - unload at configured chest before continuing")
    save(i + 1)
    return
  end
  if i % cfg.SAVE_EVERY == 0 then
    save(i + 1)
    say("Progress: " .. math.floor(i * 100 / #plan.blocks) .. "%")
  end
end
